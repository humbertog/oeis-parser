...
sub createAllLinks {
	...
	close FILE;
	open (FILE, "> ./db_owl/XML_references.rdf") or die "problem opening ./db_owl/XML_references.rdf\n";
	close(FILE);
	foreach my $seq (@content_array) {
		my @data = Parser::parseSequence("./db/sequences/$seq.txt", \&Parser::getReferences);
		@data = Util::unique(\@data);
		if ($#data >= 0) {
			my $seqXMLset="<owl:NamedIndividual rdf:about=\"http://www.semanticweb.org/humberto/ontologies/2015/2/Sequence.owl#$seq\">\n";
			foreach my $ln (@data) {
				$seqXMLset .= "<hasCrossRef rdf:resource=\"http://www.semanticweb.org/humberto/ontologies/2015/2/Sequence.owl#$ln\"/>\n";
			}
			$seqXMLset .= "</owl:NamedIndividual>\n\n";
			open (FILE, ">> ./db_owl/XML_references.rdf") or die "problem opening ./db_owl/XML_references.rdf\n";
			print FILE"$seqXMLset";
